...
__asm void os_set_env(void)
{
    ...
    PSP = MSP
          ...
          LDR     R0, = __cpp(&os_flags)
                        ...
                        BNE     PrivilegedE
                        MOVS    R0, #0x03                ;
    Unprivileged Thread mode, use PSP
    MSR     CONTROL, R0
    BX      LR
    PrivilegedE
    MOVS    R0, #0x02                ;
    Privileged Thread mode, use PSP
    MSR     CONTROL, R0
    BX      LR
...
for STRB offset < 32
    ...
    os_tsk.run = os_tsk.new
                 ...
                 ADDS    R2, #32                  ;
    for STRB offset < 32
    LDR     R3, [R2, #TCB_TSTACK - 32]  ;
    os_tsk.new->tsk_stack
    ADDS    R3, R3, #16               ;
    Adjust Start Address
    LDMIA   R3!, {R4 - R7}             ;
    ...
    LDRB    R0, [R2, #TCB_RETUPD - 32]  ;
    Update ret_val ?
    CMP     R0, #0
    BEQ     SVC_Return
    LDRB    R0, [R2, #TCB_RETVAL - 32]  ;
    Write os_tsk.new->ret_val

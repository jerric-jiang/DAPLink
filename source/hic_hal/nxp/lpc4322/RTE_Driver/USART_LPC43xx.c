...
int32_t USART_SetBaudrate(uint32_t baudrate, USART_RESOURCES *usart)
{
        ...
        else if (usart->uart_reg == NULL)
        {
            ...
            else
            {
                ...
                do
                {
                    ...
                    if (div > oversampling)
                    {
                        delta = div - oversampling;
                    }
                    else
                    {
                        delta = oversampling - div;
                    }
    ...
    if (((delta_best * 100U) / div) > USART_MAX_BAUDRATE_ERROR)
    {
        return -1;
    }
                             ...
                             USART_RESOURCES  *usart)
{
    ...
    switch (control & ARM_USART_CONTROL_Msk)
    {
        ...
        case ARM_USART_SET_IRDA_PULSE:
            if (usart->capabilities.irda)
            {
                ...
                else
                {
                    ...
                    if (arg <= (2U   * val))
                    {
                        icr |= (0U << USART_ICR_PULSEDIV_POS);
                    }
                    else if (arg <= (4U   * val))
                    {
                        icr |= (1U << USART_ICR_PULSEDIV_POS);
                    }
                    else if (arg <= (8U   * val))
                    {
                        icr |= (2U << USART_ICR_PULSEDIV_POS);
                    }
                    else if (arg <= (16U  * val))
                    {
                        icr |= (3U << USART_ICR_PULSEDIV_POS);
                    }
                    else if (arg <= (32U  * val))
                    {
                        icr |= (4U << USART_ICR_PULSEDIV_POS);
                    }
                    else if (arg <= (64U  * val))
                    {
                        icr |= (5U << USART_ICR_PULSEDIV_POS);
                    }
                    else if (arg <= (128U * val))
                    {
                        icr |= (6U << USART_ICR_PULSEDIV_POS);
                    }
                    else if (arg <= (256U * val))
                    {
                        icr |= (7U << USART_ICR_PULSEDIV_POS);
                    }
                    else
                    {
                        return ARM_DRIVER_ERROR;
        ...
        case ARM_USART_CONTROL_SMART_CARD_NACK:
            if (usart->capabilities.smart_card)
            {
                if (arg)
                {
                    usart->reg->SCICTRL &= ~USART_SCICTRL_NACKDIS;
                }
                else
                {
                    usart->reg->SCICTRL |=  USART_SCICTRL_NACKDIS;
    ...
    switch (control & ARM_USART_DATA_BITS_Msk)
    {
        case ARM_USART_DATA_BITS_5:
            lcr |= (0U << USART_LCR_WLS_POS);
            break;
        case ARM_USART_DATA_BITS_6:
            lcr |= (1U << USART_LCR_WLS_POS);
            break;
        case ARM_USART_DATA_BITS_7:
            lcr |= (2U << USART_LCR_WLS_POS);
            break;
        case ARM_USART_DATA_BITS_8:
            lcr |= (3U << USART_LCR_WLS_POS);
            break;
        default:
                                     ...
                                     USART_RESOURCES         *usart)
{
    ...
    if (control == ARM_USART_RTS_CLEAR)
    {
        if (usart->capabilities.rts)
        {
            usart->uart_reg->MCR &= ~UART_MCR_RTSCTRL;
        }
        else
        {
            return ARM_DRIVER_ERROR_UNSUPPORTED;
    ...
    if (control == ARM_USART_DTR_CLEAR)
    {
        if (usart->capabilities.dtr)
        {
            usart->uart_reg->MCR &= ~UART_MCR_DTRCTRL;
        }
        else
        {
            return ARM_DRIVER_ERROR_UNSUPPORTED;
...
static ARM_USART_MODEM_STATUS USART_GetModemStatus(USART_RESOURCES *usart)
{
        ...
        (usart->info->flags & USART_FLAG_CONFIGURED))
    {
        ...
        if (usart->capabilities.cts)
        {
            modem_status.cts = (msr & UART_MSR_CTS ? (1U) : (0U));
        }
        else
        {
            modem_status.cts = 0U;
        ...
        if (usart->capabilities.dsr)
        {
            modem_status.dsr = (msr & UART_MSR_DSR ? (1U) : (0U));
        }
        else
        {
            modem_status.dsr = 0U;
        ...
        if (usart->capabilities.ri)
        {
            modem_status.ri  = (msr & UART_MSR_RI  ? (1U) : (0U));
        }
        else
        {
            modem_status.ri  = 0U;
        ...
        if (usart->capabilities.dcd)
        {
            modem_status.dcd = (msr & UART_MSR_DCD ? (1U) : (0U));
        }
        else
        {
            modem_status.dcd = 0U;
...
static void USART_IRQHandler(USART_RESOURCES *usart)
{
    ...
    if ((iir & USART_IIR_INTSTATUS) == 0U)
    {
            ...
            ((iir & USART_IIR_INTID_MSK) == USART_IIR_INTID_CTI))
        {
            ...
            {
                case USART_TRIG_LVL_1:
                    i = 1U;
                    break;
                case USART_TRIG_LVL_4:
                    i = 3U;
                    break;
                case USART_TRIG_LVL_8:
                    i = 7U;
                    break;
                case USART_TRIG_LVL_14:
                    i = 13U;
                    break;
            }
